/**
 * Phase 0D.1 / 0D.2 — T2 narrative profile seeder
 *
 * Inserts/updates souvera_country_profiles for all 12 rollout markets with
 * institutional-grade, non-numeric narrative prose (SDC T2 tier).
 *
 * Fields populated:
 *   summary_md            – one-paragraph country overview
 *   why_now_md            – why this market matters right now for investors/policy
 *   opportunity_thesis_md – primary investment/engagement corridors
 *   risk_narrative_md     – material risks (honest, not softened)
 *   signal_level          – souvera_signal_level enum
 *   economic_momentum     – qualitative momentum descriptor
 *   investor_readiness    – qualitative readiness descriptor
 *
 * Rules (SDC T2):
 *   - No bare numeric facts (numbers live in T1 / souvera_country_observations)
 *   - Template tokens {{GDP_NOMINAL_USD}} are handled by hydration layer at render
 *   - risk_narrative_md must not be promotional or soften material risks
 *
 * Run:
 *   npx tsx --tsconfig services/ingestion/tsconfig.json \
 *     services/ingestion/run.ts seed:t2-profiles
 */

import { getSupabaseServiceClient } from '@souvera/config';

type SignalLevel = 'high_growth' | 'emerging' | 'stable' | 'watch' | 'restricted';
...
// ── Canonical T2 profiles for 12 rollout markets ─────────────────────────

const PROFILES: T2Profile[] = [
  // ── NGA — Nigeria ──────────────────────────────────────────────────────
  {
    iso3: 'NGA',
    ...
    summary_md: `Africa's most populous nation and one of the continent's largest economies. Nigeria is simultaneously the region's consumer goods gravitational centre, its leading fintech innovation hub, and a major hydrocarbons exporter — three roles that make it impossible to ignore for institutional investors operating in Africa. The non-oil private sector — spanning digital payments, fast-moving consumer goods, agro-processing, and manufacturing — is expanding structurally, reducing (though not eliminating) the economy's traditional crude-price dependence.`,
  ...
  // ── JAM — Jamaica ──────────────────────────────────────────────────────
  {
    ...
    investor_readiness: 'high',
    summary_md: `The Caribbean's leading services-led economy and a regional anchor for Business Process Outsourcing, financial services, and premium tourism. Jamaica has achieved one of the most successful post-IMF programme fiscal consolidations in the region — moving from persistent deficits to primary surpluses — and its debt-to-GDP ratio is on a demonstrable downward trajectory. CBI and CARICOM frameworks provide preferential US market access that anchors the services export corridor and supports manufacturing diversification.`,
  ...
  // ── ETH — Ethiopia ─────────────────────────────────────────────────────
  {
    ...
    summary_md: `Africa's second most populous nation and the continent's fastest-growing large economy over the prior decade before the Tigray conflict. Ethiopia's industrial park model — anchored by Hawassa Industrial City and the Eastern Industrial Zone — attracted global fast-fashion and apparel brands seeking African manufacturing alternatives, establishing a manufacturing workforce and infrastructure base that is unique on the continent. Post-conflict stabilisation and reform under Prime Minister Abiy Ahmed's administration are creating conditions for a potential second phase of industrial expansion, conditional on AGOA eligibility restoration.`,
  ...
  // ── CIV — Côte d'Ivoire ────────────────────────────────────────────────
  {
    ...
    economic_momentum: 'improving',
    investor_readiness: 'high',
    summary_md: `West Africa's largest economy and the world's leading cocoa producer, anchoring agricultural commodity flows for the entire ECOWAS zone. Abidjan — the region's most developed port city and financial centre — functions as the operational hub for multinational companies seeking ECOWAS-wide distribution, making Côte d'Ivoire the default entry point for consumer goods and industrial operators in Francophone West Africa. The country's decade-long political stabilisation under President Ouattara has delivered some of the highest sustained growth rates in the region, funded by infrastructure investment and commodity revenue management.`,
...
export async function seedRolloutT2Profiles(): Promise<void> {
  ...
  for (const p of PROFILES) {
    const countryId = countryMap.get(p.iso3);
    if (!countryId) {
      console.warn(`  ⚠  ${p.iso3} not found in souvera_countries — skipping`);
      failed++;
      continue;
    }
    ...
    const { error } = await supabase.from('souvera_country_profiles').upsert(
      {
        ...
        why_now_md: p.why_now_md,
        opportunity_thesis_md: p.opportunity_thesis_md,
        risk_narrative_md: p.risk_narrative_md,
        signal_level: p.signal_level,
        economic_momentum: p.economic_momentum,
        investor_readiness: p.investor_readiness,
    ...
    if (error) {
      console.error(`  ✗  ${p.iso3}: ${error.message}`);
      failed++;
    } else {
      console.log(`  ✓  ${p.iso3} — ${p.signal_level} | ${p.economic_momentum} | readiness: ${p.investor_readiness}`);
      upserted++;
    }
  }

  console.log(`\n  Summary: ${upserted} upserted, ${failed} failed`);
  if (failed > 0) throw new Error(`${failed} profile(s) failed to upsert`);
  console.log('\n[seed:t2-profiles] Done. T2 profiles are live in souvera_country_profiles.\n');
}
