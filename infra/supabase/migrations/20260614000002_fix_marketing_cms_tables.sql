...
-- Marketing CMS Tables Fix Migration
-- Owner: Afronovation, Inc.
-- Created: June 14, 2026
-- Note: This fixes any existing tables with wrong schema
-- ===========================================

-- Drop existing tables if they have wrong schema
-- (This will lose any existing data - only run in development)

DROP TABLE IF EXISTS souvera_marketing_audit_log CASCADE;
DROP TABLE IF EXISTS souvera_feature_flags CASCADE;
DROP TABLE IF EXISTS souvera_trust_logos CASCADE;
DROP TABLE IF EXISTS souvera_pricing_display CASCADE;
DROP TABLE IF EXISTS souvera_flash_banners CASCADE;
DROP TABLE IF EXISTS souvera_hero_slides CASCADE;

-- Drop the function if it exists
DROP FUNCTION IF EXISTS update_marketing_cms_updated_at() CASCADE;
...
CREATE POLICY "feature_flags_public_read" ON souvera_feature_flags
  FOR SELECT TO public USING (scope = 'global' AND is_enabled = true);
...
CREATE TABLE souvera_marketing_audit_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TRIGGER trigger_hero_slides_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION update_marketing_cms_updated_at();
...
-- Success
-- ============================================
SELECT 'Marketing CMS tables recreated successfully' as status;
