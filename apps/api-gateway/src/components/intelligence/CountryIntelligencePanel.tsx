...
import { 
  ...
  Calendar, Database, TrendingUp, Network, Download
} from 'lucide-react';
...
export function CountryIntelligencePanel({
  ...
  defaultPanelSubtitle = 'Ranked by Gross Domestic Product (GDP)',
  ...
  // Ref for Top 10 list export
  const top10ListRef = useRef<HTMLDivElement>(null);
  
  // Ref for country panel export
  const countryPanelRef = useRef<HTMLDivElement>(null);

  // Download handler for Top 10 list
  const handleDownloadTop10 = async () => {
    if (!top10ListRef.current) return;
    
    try {
      const regionName = workspaceRegion === 'caribbean' 
        ? 'Caribbean' 
        : 'African';
      const date = new Date().toISOString().split('T')[0];
      
      await exportElementToPNG({
        element: top10ListRef.current,
        fileName: `souvera-top-10-${workspaceRegion}-economies-${date}.png`,
        cardTitle: `Top 10 ${regionName} Economies`,
        sourceAttribution: 'World Bank · REST Countries API · SOUVERA Intelligence',
        dataAsOf: new Date().toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
      });
    } catch (err) {
      console.error('Failed to export Top 10 list:', err);
    }
  };
  
  // Download handler for country panel (entitlement-aware)
  const handleDownloadCountryPanel = async () => {
    if (!countryPanelRef.current || !data) return;
    
    try {
      const date = new Date().toISOString().split('T')[0];
      const sources = data.meta.sources.map(s => s.name).join(' · ');
      
      await exportElementToPNG({
        element: countryPanelRef.current,
        fileName: `souvera-${data.country.iso3.toLowerCase()}-intelligence-${date}.png`,
        countryName: data.country.name,
        iso2: data.country.iso2,
        flagUrl: data.country.flagUrl,
        cardTitle: 'Market Intelligence',
        sourceAttribution: `${sources} · SOUVERA Intelligence`,
        ...
        disclaimer: 'This export respects your plan entitlements. Locked data is not included.',
      });
    } catch (err) {
      console.error('Failed to export country panel:', err);
    }
  };
      ...
      return (
        <div className="h-full bg-zinc-900/40 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
          <div className="p-5 border-b border-zinc-800 shrink-0">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-zinc-400 hover:text-white hover:bg-zinc-800 rounded transition-colors"
                data-export-exclude
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {/* Column Headers */}
            <div className="px-5 py-2 border-b border-zinc-800/50 grid grid-cols-[auto_1fr_auto] gap-3 text-xs font-semibold text-zinc-500 sticky top-0 bg-zinc-900/40 backdrop-blur-sm">
              <span className="w-7"></span>
              <span>Country</span>
              <span className="text-right">GDP (2023)</span>
            </div>
            
            <div ref={top10ListRef} className="divide-y divide-zinc-800/60">
  ...
  return (
    <div className="h-full bg-zinc-900/80 border border-zinc-700 rounded-xl overflow-hidden flex flex-col">
      ...
      <div className="p-5 border-b border-zinc-800 shrink-0">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2" data-export-exclude>
            <button
              onClick={handleDownloadCountryPanel}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-zinc-400 hover:text-white hover:bg-zinc-800 rounded transition-colors"
              title="Download as PNG"
            >
              <Download className="w-3.5 h-3.5" />
      ...
      {/* Scrollable content */}
      <div ref={countryPanelRef} className="flex-1 overflow-y-auto">
        {/* Data status banner */}
