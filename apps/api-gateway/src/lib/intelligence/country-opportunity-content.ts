...
function jamaicaOpportunity(countryName: string): CountryOpportunityContent {
  return {
    ...
    heroFallback: `${countryName} is a $19B Caribbean economy at a digital inflection point, offering opportunities across tourism, digital infrastructure, and mining/energy transition.`,
    pillars: [
      {
        exportId: 'tech-pillar-card',
...
function kenyaOpportunity(countryName: string): CountryOpportunityContent {
  return {
    ...
    heroFallback: `${countryName} is a $115B East African economy at a digital and infrastructure inflection point, offering opportunities across fintech, renewables, agriculture, and logistics gateway investment.`,
    pillars: [
      {
        exportId: 'tech-pillar-card',
...
function trinidadOpportunity(countryName: string): CountryOpportunityContent {
  return {
    ...
    heroFallback: `${countryName} is a $28B CARICOM economy combining LNG, petrochemicals, and manufacturing scale with CBI preferential U.S. access.`,
    pillars: [
      {
...
function barbadosOpportunity(countryName: string): CountryOpportunityContent {
  return {
    ...
    heroFallback: `${countryName} offers stable governance, CBI market access, and premium tourism recovery on a $6.5B Eastern Caribbean platform.`,
    pillars: [
      {
...
function bahamasOpportunity(countryName: string): CountryOpportunityContent {
  return {
    ...
    heroFallback: `${countryName} is a $14B archipelago economy combining tourism, IFS, and Freeport logistics with CBI preferential U.S. access.`,
    pillars: [
      {
        exportId: 'tourism-pillar-card',
        exportFileSlug: 'tourism-opportunity',
    ...
    regionalAdvantages: [
      ...
      { icon: 'users', value: '$14B', label: 'Economic Scale', sublabel: 'Largest CARICOM island GDP', accentClass: 'text-emerald-300' },
    ],
  };
}
...
export function getOpportunityContent(iso3: string, countryName: string): CountryOpportunityContent {
  ...
  if (['GHA', 'ZAF', 'ETH', 'SEN', 'CIV', 'TZA'].includes(key)) return wave1AfricaOpportunity(key, countryName);
...
function wave1AfricaOpportunity(iso3: string, countryName: string): CountryOpportunityContent {
  ...
  return {
    ...
    pillars: [
      { exportId: 'tech-pillar-card', exportFileSlug: 'sector-opportunity-1', exportTitle: 'Primary Sector Opportunity', icon: 'zap', title: 'Primary Sector', subtitle: 'Growth opportunity', narrative: p.pillar1, bullets: [{ label: 'Sector analysis', text: 'See Sectors tab for strength, growth, and attractiveness scores' }], borderHover: 'hover:border-blue-500/30', accentClass: 'text-blue-400', helpTerm: 'tech_sector_opportunity' },
      { exportId: 'agriculture-pillar-card', exportFileSlug: 'sector-opportunity-2', exportTitle: 'Secondary Sector Opportunity', icon: 'trending', title: 'Secondary Sector', subtitle: 'Emerging opportunity', narrative: p.pillar2, bullets: [{ label: 'Macro context', text: 'GDP growth, FDI, and inflation trends inform sector timing' }], borderHover: 'hover:border-emerald-500/30', accentClass: 'text-emerald-400', helpTerm: 'agriculture_opportunity' },
      { exportId: 'infrastructure-pillar-card', exportFileSlug: 'sector-opportunity-3', exportTitle: 'Infrastructure Opportunity', icon: 'building', title: 'Infrastructure & Logistics', subtitle: 'Development pipeline', narrative: p.pillar3, bullets: [{ label: 'Market access', text: 'See Trade tab for bilateral and regional frameworks' }], borderHover: 'hover:border-amber-500/30', accentClass: 'text-amber-400', helpTerm: 'infrastructure_opportunity' },
    ],
    entryPoints: [
